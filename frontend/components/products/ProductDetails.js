...
export default function ProductDetails({ data, shortName, features, includedItems, gallery, others, _id, reviews }) {
    ...
    return (
        <div className='ProductDetails'>
            ...
            <div>
                ...
                <form action={`http://localhost:5500/api/${_id}/reviews`} method='POST'>
                    <label htmlFor='rating'>rating</label>
                    <input id='rating' type='range' name='rating' min={1} max={5} defaultValue={1} />
                    <label htmlFor='body'>Review</label>
                    <textarea id='body' name='body' rows={3} cols={40}>
                    </textarea>
                    <button type='submit' className='Btn--orange'>Save</button>
                </form>
                {reviews.length === 0 ? null : reviews.map((review, i) => <div key={i}>
                    <h1>{review.rating}</h1>
                    <p>{review.body}</p>
                    <form action={`http://localhost:5500/api/${_id}/reviews/${review._id}?_method=DELETE`} method='POST'><button>Delete</button></form>
                </div>)}
