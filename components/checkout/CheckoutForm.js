...
export default function CheckoutForm() {
    ...
    const handleSubmit = e => {
        e.preventDefault();
        [...e.target.elements].map(ele => {
            if (ele.tagName === 'INPUT') ele.focus(), ele.blur();
        });
        if (
            [...e.target.elements].every((ele, i) => {
        ...
        <form className='CheckoutForm' onSubmit={handleSubmit} noValidate>
            <div className='CheckoutForm__container'>
                ...
                <fieldset className='CheckoutForm__billing'>
                    ...
                    <legend className='CheckoutForm__subtitle'>
            ...
            {!isSubmitted ? null : <Confirmation isSubmitted={isSubmitted} />}
        </form>
    );
}
